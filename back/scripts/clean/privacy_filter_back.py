...
class PrivacyFilter:
    ...
    def __init__(
        ...
        print("\n" + "=" * 70)
        print("🔒 KLUE + GLiNER 하이브리드 필터 (T5 통합 버전)")
        print("=" * 70)
    ...
    def filter_text(
        ...
        filtered_text = self._remove_detections(text, merged)

        # ⚠️ 최소한의 정리만 수행 (T5가 나머지 처리)
        filtered_text = self._minimal_cleanup(filtered_text)
